...
const LandCard: React.FC<LandCardProps> = ({ listing, onClick }) => {
  ...
  // Helper to render specific SVG icons based on feature text
  ...
  return (
    ...
    >
      {/* Image Section with Graphics */}
      <div className="relative h-52 overflow-hidden bg-stone-100">
        {!imgError ? (
            <img 
                src={listing.imageUrl || "https://placehold.co/600x400?text=Konaki"} 
                alt={listing.type}
        ...
        <div className="absolute top-3 left-3 flex gap-2">
             <div className={`backdrop-blur-md px-3 py-1 rounded-full shadow-sm border border-white/10 flex items-center gap-1 ${isEquipment ? 'bg-orange-500/90 text-white' : 'bg-green-600/90 text-white'}`}>
                {isEquipment ? (
                   <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M19 8h-1V5h-2v3h-2V5h-2v3h-2V5H8v3H7c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM8 12h2v2H8v-2zm0 4h2v2H8v-2zm4 4h-2v-2h2v2zm0-4h-2v-2h2v2zm4 4h-2v-2h2v2zm0-4h-2v-2h2v2z"/></svg>
                ) : (
        ...
        <div className="absolute top-3 right-3">
             <div className="bg-white/90 backdrop-blur-md text-stone-800 px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wide">{listing.district}</span>
             </div>
        </div>
      ...
      <div className="p-5 flex flex-col flex-1 relative bg-white">
        ...
        {/* Features as Graphics (Pill Badges with Icons) */}
